...
def withdraw(wac, session: Session, form_name: str, account: Account):
    if account.kyc_completed is False:
        ...
        <input type='submit' value='Continue withdrawal' onclick="window.top.postMessage(['transfer', '%s', 'You are about to withdraw money to %s.\\nPlease enter the amount of money to withdraw\\n\\n', %s], '*');"/><br/><br/><br/>
        """ % (account.kyc_name, request.args['bank_account'], account.deposit_address, request.args['bank_account'], string_to_javascript_array(attachment))
    else:
        s = """
        Welcome %s!<br/>
...
forms = {"details": details, "kyc": kyc, "withdraw": withdraw}
